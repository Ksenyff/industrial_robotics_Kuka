...
#-----------Функция поиска координат по камере------------------------#
    def marker_coordinates(self, topic):
        ...
        #distortion
        dist = np.array([-0.063212, 0.479756, 0.007614, 0.003480, 0.0], dtype=np.float32)
        # В мм
        M1_siz = 100.0   
        M2_siz = 40.0    
        ...
        def estimate_marker_pose(corners_2d, size):
            ...
            #rvec - вида вектора Эйлера–Родрига и tvec - вектор сдвига (3 на 1)
            success, rvec, tvec = cv2.solvePnP(
                obj, corners_2d, K, dist, flags=cv2.SOLVEPNP_IPPE_SQUARE
            )
            if not success:
                success, rvec, tvec = cv2.solvePnP(
                    obj, corners_2d, K, dist, flags=cv2.SOLVEPNP_ITERATIVE
                )
            if not success:
...
def main():
    try:
        ...
        input(
            "============ сделать снимок ============")
        x, y = move_group.marker_coordinates( "/pylon_camera_node/image_raw")
        print("x: ", x)
        print("y: ", y)
